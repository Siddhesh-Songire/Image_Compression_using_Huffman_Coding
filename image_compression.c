...
int main()
{
	...
	image_file = fopen(filename, "rb"); // read in binary mode
	if (image_file == NULL)
	{
	...
	for (i = 0; i < nodes; i++){
		...
		len += codelen(pix_freq[i].code) * hist[pix_freq[i].pix] ;
	}
		printf("\nNo of bits after compression: %d",len); 
	printf("\nAverage number of bits:: %f", avgbitnum);
}
